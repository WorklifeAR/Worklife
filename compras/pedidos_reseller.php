...
if(!isset($_SESSION['sess_usu_id']) && !isset($_SESSION['reseller'])){
...
function guardar($estado,$comentarios,$id_pedido,$mail_usuario)
{
	...
	echo"<META HTTP-EQUIV='Refresh' CONTENT='0;URL=ver.php?mod=pedidos_resellers&id_pedido='".$id_pedido."'>";
	die();	
}
/****************************************************************************************************************************/
...
<form action="" name="filtro" id="filtro" method="post">
  <tr>
    <td colspan="7"><div align="center" class="titulo">My Orders <br />
    </div>
  ...
  <tr>
    ...
    <td colspan="6"><select name="estados" id="estados">
      <?php	
...
?>
  ...
  <tr>
    <td colspan="3"><input type="submit" name="fitrar2" value="Create Report" onclick="javascript:document.getElementById('filtro').action='compras/excel_pedidos.php';"/></td>
    <td align="right">&nbsp;</td>
    <td align="right">&nbsp;</td>
...
<table width="100%" border="1" cellspacing="0" cellpadding="5" bordercolor="#CCCCCC" style="border-collapse:collapse;">
  <tr bgcolor="#E6E6E6" align="center">
    <td class="titulo">Order #</td>
    <td class="titulo">Buyer</td>
...
<?php
	...
	$sql="SELECT id_pedido,id_usuario,id_articulo,nombre,email,fecha_pedido,pais,ciudad,cp,direccion,
				 ...
				 valor_envio,comentarios,reseller
	      FROM ic_pedidos 
		  WHERE  reseller='".$_SESSION['reseller']."' ".$estados.$fecha_pedido.$reseller." group by id_pedido,id_usuario ORDER BY id_pedido DESC";
	...
	while(!$result->EOF){
		list($id_pedido,$id_usuario,$id_articulo,$nombre,$email,$fecha_pedido,$pais,$ciudad,$cp,$direccion,
	         $telefono,$estado,$metodo_pago,$nombre_articulo,$cantidad,$precio_articulo,$impuesto,$envio,
			 $valor_envio,$comentarios,$reseller)=select_format($result->fields);
...
?>
  <tr>
    <td align="center"><a href="ver.php?mod=pedidos_resellers&id_pedido=<?=$id_pedido?>">Order - # <?=$id_pedido?></a></td>
    <td><?=$nombre?></td>
...
$sql="SELECT id_pedido,id_usuario,id_articulo,nombre,email,fecha_pedido,pais,ciudad,cp,direccion,
	         telefono,estado,metodo_pago,nombre_articulo,cantidad,precio_articulo,impuesto,envio,
			 valor_envio,comentarios,reseller
      FROM ic_pedidos WHERE id_pedido =".$_GET['id_pedido']." AND reseller='".$_SESSION['reseller']."'";
$result=$db->Execute($sql);
...
<div class="titulo" align="right"><a href="ver.php?mod=pedidos_resellers"><?=_BACK?></a></div>
...
?>
  ...
  <tr>
    ...
    <td bgcolor="#C5D0FE"><strong>Reseller</strong></td>
    <td bgcolor="#C5D0FE"><?=$vendedor?></td>
  </tr>
  <tr>
