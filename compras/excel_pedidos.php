<?php
	...
	$variables_metodo = variables_metodo("fecha_pedido,estados,departamento,fecha_pedido_end");
	...
	$sql="SELECT id_pedido,id_usuario,id_articulo,nombre,email,fecha_pedido,pais,ciudad,cp,
      ...
      FROM ic_pedidos WHERE reseller='".$_SESSION['reseller']."' ".$estados.$fecha_pedido.$departamento." GROUP BY id_pedido ";
...
?>
  <tr>
    ...
    <td><?=$envio?></td>
    <td><?=$valor_envio?></td>
    <td><?=($precio+$impuesto+$valor_envio)?></td>
    <td><?=$comentarios?></td>
  </tr>
