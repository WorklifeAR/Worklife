...
<?php
	...
	$variablesPaginas = variables_metodo('nombre_completo,email,ciudad,telefono,metodocontacto,comentarios,estado,phone');
	$nombre_completo=		$variablesPaginas[0];
	$email	=				$variablesPaginas[1];
	$ciudad	=				$variablesPaginas[2];
	$telefono	=			$variablesPaginas[3];
	$metodocontacto	=		$variablesPaginas[4];
	$comentarios	=		$variablesPaginas[5];
	$estado	=				$variablesPaginas[6];
	$telefono	=			$variablesPaginas[7];
	...
	$cabeceras .= "From: Worklife - Contacto Usuario <".$MailTo.">\r\n";
	$cabeceras .= "Return-path: ".$MailTo."\r\n"; 
...
----------------------------------------------------------------------------<br /><br />";
	...
	registrar_cambios($db, date('Y-m-d'), $nombre_completo, '00', "", strip_tags($contenido), 'CONTACTO');
...
<?
	if(mail($MailTo, "Worklife - Contacto Usuario", $contenido, $cabeceras)){
